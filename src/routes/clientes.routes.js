...
router.post('/', async (req,res) => {
  ...
  const [result] = await pool.query(
    'INSERT INTO cliente (nome_cliente, telefone_cliente, email_cliente, data_cadastro) VALUES (?, ?, ?, ?)',
    [nome_cliente, telefone_cliente, email_cliente, data_cadastro]
  );
  res.status(201).json({ id: result.insertId, nome_cliente, telefone_cliente, email_cliente });
});
